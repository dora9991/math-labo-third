...
//    ・れんしゅうメダル … れんしゅうで検証済みの正解を5問ためる（難度はどれでもOK）（サーバーが採点）
//    ・バトルメダル   … その小単元のバトルをはじめてクリア（サーバーが申請を検証して付与）
//  バトルはいつでも挑戦できる（メダルの条件なし）。章ボスは、その章のバトルメダルが全部そろうと開く。
...
export const hasBattleMedal = (state, unitId) => unitMedals(state, unitId).battle;

/** 単元の配列に対するメダル総数／満点／バトルクリア数 */
export function medalSummary(state, units = []) {
  let count = 0, cleared = 0;
  for (const u of units) {
    const m = unitMedals(state, u.id);
    count += m.count;
    if (m.battle) cleared += 1;
  }
  return { count, total: units.length * 3, battlesCleared: cleared, units: units.length };
}
