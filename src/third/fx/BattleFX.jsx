...
// 【数学ラボ3】弾の速度を2倍に（飛行フレーム数を半分）。React側の待ち時間の近似値もそれに合わせて半分。
export const PROJECTILE_MS = { normal: 200, crit: 250, miss: 150 };
...
const BattleFX = forwardRef(function BattleFX({ speed = "normal" }, ref) {
  ...
  speedRef.current = prefersReducedMotion() ? "off" : speed;
  ...
  // Pixi の全 ticker 演出（弾、爆発、数字、反撃、バナー）に同じ倍率を与える。
  // fxScale は「所要時間」の係数なので、ticker には逆数を渡す。off は各起動
  // メソッドで飛翔を省くため、ticker 自体は通常速度で短い着弾表示を消す。
  useEffect(() => {
    if (appRef.current) {
      const scale = fxScale(speedRef.current);
      appRef.current.ticker.speed = speedRef.current === "off" ? 1 : 1 / scale;
    }
  }, [speed]);
