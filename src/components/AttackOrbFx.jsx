...
export default function AttackOrbFx({ attack, sourceRef, targetRef, onImpact }) {
  ...
  useEffect(() => {
    ...
    const next = { ...attack, start, end, dx: end.x - start.x, dy: end.y - start.y, reduced };
    setFx(next);
    const impact = () => {
    ...
    timers.current = [
      setTimeout(impact, reduced ? 90 : 830),
      setTimeout(() => {
        document.documentElement.classList.remove("attack-impact-shake");
  ...
  return (
    <div className={`attack-orb-fx${fx.strong ? " strong" : ""}${fx.reduced ? " reduced" : ""}`} aria-hidden="true">
      ...
      {fx.impact && <>
        ...
        <div className="orb-impact" style={{ left: fx.end.x, top: fx.end.y }}>
